...
If a fresh scan immediately reports “The source or local upload directory changed,” deploy the version that decodes MariaDB JSON snapshots and resume the saved migration. No new database migration is needed for that compatibility fix. If the Spaces endpoint, region, bucket, CDN base URL, or `UPLOAD_DIR` actually changed after scanning, close the migration and scan again. Use a stable, persistent `UPLOAD_DIR` across deployments; closing a migration does not undo already copied files or updated links.
